...
class Constant {
    ...
    companion object {
        ...
        // STATES REQUESTS
        const val STATE_OPEN = 200L // заявка открыта, отправитель никого еще не подтвердил
        const val STATE_IN_PROGRESS = 201L // отправитель подтвердил бронь (перевозчику)
        const val STATE_COMPLETE =
            202L //заявка выполнена (после того как подтвердили что груз доставили)
        const val STATE_NOT_COMPLETED =
            203L // заявка не выполнена (перевозчик отменил доставку (после брони уже) тогда удаляется delivery а в request выставляется статус этот)
        const val STATE_CANCELLED =
            204L // заявка отменена (отправитель отменил перевозку - то все delivery удаляются)
